...
class mqttbroker(msgbus):
    ...
    def on_message(self, client, userdata, msg):
        ...
        message.update({'DEVICE_NAME':list_topic[-2]})
        message.update({'PORT_NAME':list_topic[-1]})
        message.update({'MESSAGE':msg.payload})
        self.msgbus_publish('LOG','%s Broker: received Date Device: %s , Port: %s , Message: %s'%('INFO',message['DEVICE_NAME'], message['PORT_NAME'], message['MESSAGE']))
